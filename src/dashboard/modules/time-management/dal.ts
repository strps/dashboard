...
import { and, asc, desc, eq, gte, inArray, isNull, lte, max, or } from "drizzle-orm";
...
function row(r: typeof activity.$inferSelect, tagIds: string[]): Activity {
...
async function activityExistsForUser(userId: string, id: string): Promise<boolean> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        lte(timeEntry.startedAt, to),
        or(isNull(timeEntry.endedAt), gte(timeEntry.endedAt, from)),
      ),
    )
