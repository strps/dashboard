...
export async function getWidgetConfigAction(
  widgetInstanceId: string,
): Promise<FilterButton[]> {
  return getWidgetConfig(widgetInstanceId);
}
...
export async function saveWidgetConfigAction(
  ...
  const parsed = filterButtonsSchema.parse(filterButtons);
  await saveWidgetConfig(widgetInstanceId, parsed);
}
