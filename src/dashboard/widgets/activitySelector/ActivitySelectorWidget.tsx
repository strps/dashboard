...
type ClockMode = "visible" | "muted" | "hide-big" | "hidden";
...
export function ActivitySelectorWidget({ id }: WidgetComponentProps) {
  ...
  const [clockMode, setClockMode] = useState<ClockMode>("visible");
  const ref = useRef<HTMLDivElement>(null);

  const cycleClockMode = () =>
    setClockMode((m) => CLOCK_MODES[(CLOCK_MODES.indexOf(m) + 1) % CLOCK_MODES.length]);
