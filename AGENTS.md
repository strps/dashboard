...
- Icons: `lucide-react`.
- DB: Drizzle ORM. Schemas can live in [src/lib/db/schema.ts](src/lib/db/schema.ts) (shared) or in a module's own `db.ts` (e.g. [time-management/db.ts](src/dashboard/modules/time-management/db.ts)). Every schema file must be listed in [drizzle.config.ts](drizzle.config.ts). Migration commands: `pnpm db:generate`, `pnpm db:migrate`, `pnpm db:push`, `pnpm db:studio`.
