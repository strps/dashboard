...
- Icons: `lucide-react`.
- DB: Drizzle ORM. Schemas can live in [src/lib/db/schema.ts](src/lib/db/schema.ts) (shared) or in a module's own `db.ts` (e.g. [time-management/db.ts](src/dashboard/modules/time-management/db.ts)). Every schema file must be listed in [drizzle.config.ts](drizzle.config.ts). Migration commands: `pnpm db:generate`, `pnpm db:migrate`, `pnpm db:push`, `pnpm db:studio`. For migrations that require custom SQL (data migrations, PK swaps, multi-step ALTER): use `pnpm db:generate --custom --name=<migration_name>` — this generates an empty SQL file for you to fill in manually, rather than auto-generating from schema diff.
